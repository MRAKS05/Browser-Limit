...
The parental lock provides a basic level of protection against impulsive changes. The PIN prevents unauthorized configuration changes and exception modifications. However, the PIN is stored in plaintext in SharedPreferences, which is a known limitation ([#6](https://github.com/MRAKS05/Browser-Limit/issues/6)).
