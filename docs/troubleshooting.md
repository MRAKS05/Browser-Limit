...
### Service state toggle is inverted

**Symptoms:**
- The notification says "Paused" when the service is active, or "Active" when it is paused
- After restarting the app, the active state is the opposite of what you set

**Cause:**
- A known bug ([#7](https://github.com/MRAKS05/Browser-Limit/issues/7)): the Dashboard and GuardService both try to set the active state, resulting in a double negation.

**Workaround:**
- Toggle the switch twice to correct the state. This is a temporary workaround.
- The fix is tracked in the linked issue.
...
### Overlay shows "Browser Removed" before removal

**Symptoms:**
- The overlay heading says "Browser Removed" but the app is still installed
- The countdown is still running or the "Keep" button is still available

**Cause:**
- A known UI bug ([#8](https://github.com/MRAKS05/Browser-Limit/issues/8)): the overlay displays past-tense text before the uninstall actually happens. The app is only removed when the countdown expires or you tap "Remove Now".

**Workaround:**
- Ignore the text — the app has not been removed yet. You still have time to tap "Keep & Add Exception".
...
### Dashboard counts are stuck after midnight

**Symptoms:**
- The scanned and removed counts show the same values all day
- The counts only update when you restart the app

**Cause:**
- A known bug ([#9](https://github.com/MRAKS05/Browser-Limit/issues/9)): the daily timestamp is calculated once when the Dashboard first opens and is never recalculated. If the app stays open past midnight, the counts freeze.

**Workaround:**
- Close and reopen the app to reset the daily counters.
...
### Want to improve browser detection?

If you encounter a false positive (non-browser removed) or false negative (browser not detected), you can help improve the app:

1. **Add the app to exceptions** — Prevents future removal of a false positive.
2. **Recheck with Gemini** — From the Logs screen, forces reclassification and updates the cache.
3. **Submit feedback** — A feedback system ([#5](https://github.com/MRAKS05/Browser-Limit/issues/5)) is planned to let you flag incorrect classifications directly from the log, triggering a Gemini re-evaluation and cache correction.
